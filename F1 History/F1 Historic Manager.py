...
def generate_new_driver():
    ...
    eligible_drivers = df[df["Age"] > 17].dropna(subset=["Driver name", "Current Rating"])

    if eligible_drivers.empty:
        print("No eligible drivers found.")
        return None

    selected_driver = eligible_drivers.sample(n=1).iloc[0]
    name = selected_driver["Driver name"]
    age = calculate_age(selected_driver["Date of Birth"])
    skill = selected_driver["Current Rating"] 
    aggression = random.randint(40, 90)
    consistency = random.randint(50, 100)

    return Driver(name, age, skill, aggression, consistency)
...
def simulate_race(teams):
    ...
    fastest_lap = random.randint(1,56)
    if fastest_lap == 1: #1 ≈2%
        fastest_driver = 10
    elif 1 < fastest_lap <=3: #2-3 ≈4%
        fastest_driver = 9
    elif 3 < fastest_lap <=6: #4-6 ≈5%
        fastest_driver = 8
    elif 6 < fastest_lap <=10: #7-10 ≈7%
        fastest_driver = 7
    elif 10 < fastest_lap <=15: #11-15 ≈9%
        fastest_driver = 6
    elif 15 < fastest_lap <=21: #16-21 ≈11%
        fastest_driver = 5
    elif 21 < fastest_lap <=28: #22-28 ≈13%
        fastest_driver = 4
    elif 28 < fastest_lap <=36: #29-36 ≈15%
        fastest_driver = 3
    elif 36 < fastest_lap <=45: #37-45 ≈16%
        fastest_driver = 2
    elif 45 < fastest_lap <=55: #46-55 ≈18%
        fastest_driver = 1
    for index, result in enumerate(results):
        ...
        #Fastest lap will always be top 10
        if position == fastest_driver:
            points += 1
...
def race_simulation_window():
    ...
    ttk.Label(race_window, text=("\U0001F3C1Race Results for the " + tracks_1950[current_race_index]), font=("Arial", 16, "bold")).pack(pady=10)
...
TO-DO List
Age
-Drivers age is currently worked out as of todays date not the date in game
...
-Save season data
-Be able to use other made functions, - retiring drivers
-Historical point system
-Include rating increase (Potential)
'''

'''
Updates:
-Linked excel sheet so now the driver rating is taken from there instead of randomly generated.
-Historically better drivers will now likely be better depending on the car.

-Fastest lap now isn't always given to first.
-Randomly allocated to a driver in the top 10, higher you finish the more likely it is that you get it.

-Update driver data in excel spreadsheet to the end of the 2024 season (Not including 2025)
-Created a new formula for the driver rating (Worse drivers have a better chance of getting better)
'''